    ...
    methods: {
      ...
      async validateField() {
        ...
        if(state === false) {
          ...
          const rekey = this.phone+'/'+this.group+'/'+this.resume.name+'/'+this.name.split(' ').join('-')+'简历.'+rel[rel.length-1]
          ...
          if (this.work) {
            const workkey = this.phone+'/'+this.group+'/'+this.work.name+'/'+this.name.split(' ').join('-')+'-'+this.work.name.split(' ').join('-')
